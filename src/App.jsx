...
export default function App() {
  ...
  return (
    <div className="min-h-screen transition-colors bg-gradient-to-br from-[#e0f2fe] via-[#f8fafc] to-[#e0e7ff]">
      ...
      <nav className="fixed top-0 left-0 w-full z-50 bg-white/70 backdrop-blur-xl shadow-md border-b border-[#e0f2fe] transition-all">
        <div className="max-w-6xl mx-auto flex items-center justify-between px-6 py-3">
          <div className="flex items-center gap-3">
            ...
            <span className="text-xl font-extrabold text-[#2563eb] tracking-tight drop-shadow-sm">Sergio Sabater</span>
          </div>
          <div className="flex items-center gap-1 sm:gap-3">
            <a href="#sobre-mi" className="relative px-3 py-1 rounded-lg font-semibold text-[#2563eb] hover:bg-[#e0f2fe] transition-colors duration-200">
              {t.nav[0]}
            </a>
            <a href="#proyectos" className="relative px-3 py-1 rounded-lg font-semibold text-[#2563eb] hover:bg-[#e0f2fe] transition-colors duration-200">
              {t.nav[1]}
            </a>
            <a href="#contacto" className="relative px-3 py-1 rounded-lg font-semibold text-[#2563eb] hover:bg-[#e0f2fe] transition-colors duration-200">
              {t.nav[2]}
            </a>
            <button
              className="ml-2 rounded-lg px-3 py-1 bg-[#e0f2fe] text-[#2563eb] font-semibold border border-[#b3e0ff] hover:bg-[#bae6fd] transition-colors"
      ...
      {/* HERO + FOTO */}
      <header className="relative flex flex-col items-center justify-center min-h-[35vh] py-10 px-4 bg-transparent pt-20">
        ...
        >
          <a href="mailto:sergio.telematico@gmail.com" className="hover:underline text-[#1a8cff] font-bold">{t.hero.mail}</a>
          <a href="https://www.linkedin.com/in/sergio-sabater-ruiz-562753268/" className="hover:underline text-[#1a8cff] font-bold">{t.hero.linkedin}</a>
        </motion.div>
      </header>

      {/* SOBRE MI */}
      <motion.section
        className="max-w-3xl mx-auto -mt-10 mb-8 bg-white/70 rounded-2xl p-8 shadow-2xl border border-[#cce6ff] relative z-20"
      ...
      <main className="max-w-5xl mx-auto grid grid-cols-1 md:grid-cols-2 gap-7 px-2 z-30">
        {t.blocks.map((block, idx) => (
          <motion.section
            key={block.title}
            className="rounded-2xl p-7 bg-white/70 shadow-xl mb-2 border border-[#cce6ff] hover:shadow-2xl hover:-translate-y-1 hover:scale-[1.025] transition-all duration-300"
            initial={{ opacity: 0, y: 60, scale: 0.97 }}
            whileInView={{ opacity: 1, y: 0, scale: 1 }}
            viewport={{ once: true }}
        ...
        {/* Skills & Soft Skills */}
        <motion.section
          className="col-span-1 rounded-2xl p-7 bg-white/70 shadow-xl flex flex-col gap-2 border border-[#cce6ff] hover:shadow-2xl hover:-translate-y-1 hover:scale-[1.025] transition-all duration-300"
          initial={{ opacity: 0, y: 60, scale: 0.97 }}
          whileInView={{ opacity: 1, y: 0, scale: 1 }}
          viewport={{ once: true }}
          ...
          whileHover={{ scale: 1.025, boxShadow: "0 6px 40px #1a8cff20" }}
        >
          <h3 className="text-xl font-bold text-[#195799] mb-2">{t.skills}</h3>
          ...
          <h3 className="text-xl font-bold text-[#195799] mt-3 mb-1">{t.softSkills}</h3>
          ...
          <h3 className="text-xl font-bold text-[#195799] mt-3 mb-1">{t.languages}</h3>
          <div className="flex flex-wrap gap-2">
        ...
        <motion.section
          ...
          id="proyectos"
        >
          <h3 className="text-xl font-bold text-[#195799] mb-2">{t.projects}</h3>
          ...
          <h3 className="text-xl font-bold text-[#195799] mb-2 mt-1">{t.certs}</h3>
      ...
      <motion.section
        className="max-w-5xl mx-auto my-10 rounded-3xl p-0 sm:p-10 bg-white/70 shadow-2xl flex flex-col md:flex-row items-center gap-7 overflow-hidden relative border-2 border-[#a6d9ff]"
      ...
      >
        <div className="flex-1 py-6 px-4">
          ...
          >
            <span className="block font-bold text-[#195799] text-xl">
              {t.award1.title} <span className="text-[#ffc300] text-lg">{t.award1.year}</span>
            </span>
            <span className="block text-base text-[#1a8cff]/80">{t.award1.desc}</span>
          </motion.div>
          <motion.div
            initial={{ opacity: 0, x: 50 }}
            whileInView={{ opacity: 1, x: 0 }}
          ...
          >
            <span className="block font-bold text-[#195799] text-xl mt-3">
              {t.award2.title} <span className="text-[#ffc300] text-lg">{t.award2.year}</span>
            </span>
            <span className="block text-base text-[#1a8cff]/80">{t.award2.desc}</span>
          </motion.div>
        </div>
        <div className="flex-1 flex flex-col items-center justify-center py-6 px-4">
          ...
          >
            <img src={FOTO_PREMIO} alt={t.premioFoto} className="object-cover w-full h-64 transition-all duration-500" />
          </motion.div>
          <motion.span
