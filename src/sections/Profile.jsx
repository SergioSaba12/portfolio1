          ...
          <div className="md:col-span-3">
            ...
            <div className="mt-6 grid md:grid-cols-2 gap-6">
              <div>
                ...
                <div className="mt-2 flex flex-wrap gap-2">
                  {person.languages.map((l,i)=>(<span key={i} className="apple-chip">{l}</span>))}
                </div>
                <h3 className="mt-6 text-sm font-semibold tracking-wide uppercase apple-muted">Links</h3>
                <div className="mt-2 flex flex-wrap gap-2">
                  {person.links.map((l,i)=>(
                    <a key={i} href={l.href} target="_blank" rel="noreferrer noopener" className="apple-chip">{l.label}</a>
                  ))}
                </div>
              </div>
              <div>
                ...
                <ul className="mt-2 space-y-1 text-sm">
                  {person.awards.map((a,i)=>(<li key={i}>• {a}</li>))}
                </ul>
              </div>
            </div>
            ...
            <div className="mt-6 grid md:grid-cols-2 gap-6">
              ...
              <div>
                ...
                <div className="mt-2 flex flex-wrap gap-2">
                  {person.skills.map((s,i)=>(<span key={i} className="apple-chip">{s}</span>))}
                </div>
              </div>
            </div>
          </div>
